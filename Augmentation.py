...
if __name__ == '__main__':
    ...
    if os.path.isdir(args.path):
        ...
        save_dir = 'augmented_directory/'
        if args.path == 'Apple':
            save_dir = os.path.join(save_dir, 'Apple/')
        if args.path == 'Grape':
            save_dir = os.path.join(save_dir, 'Grape/')
        for image in tqdm(images,
                          desc=f'Copying images from {args.path} \
                            to augmented_directory',
                          total=len(list(Path(args.path).glob('**/*.JPG')))):
            save_path = Path(
                save_dir,
                image.parent.stem)
        ...
        len_largest_directory = len(list(largest_directory.iterdir())) + 400
