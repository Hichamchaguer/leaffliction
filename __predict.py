#!/usr/bin/env python3
"""
predict.py — Part 4.2 of the Leaffliction project.

Loads the model trained by train.py and either:
  (a) classifies a single image, displaying original + transformed
      side by side with the predicted disease class, or
  (b) batch-tests accuracy over N random images sampled recursively
      from a folder, printing a running accuracy percentage.

Usage:
    Single image:
        python3 predict.py path/to/leaf.JPG

    Custom model/labels paths:
        python3 predict.py path/to/leaf.JPG --model models/model.h5 \
            --labels models/labels.txt

    Batch accuracy test (up to N random images from a folder):
        python3 predict.py path/to/folder --batch-test 100
"""
...
def get_transformed_panel(image_path, image_size):
    ...
    try:
        ...
        transformer = ImageTransformation(str(image_path))
        transformer._get_mask()
        transformer._mask_img()

        if transformer.clean_mask.ndim == 2:
            transformer.clean_mask = cv2.cvtColor(transformer.clean_mask,
                                                  cv2.COLOR_GRAY2RGB)
        else:
            transformer.clean_mask = cv2.cvtColor(transformer.clean_mask,
                                                  cv2.COLOR_BGR2RGB)
        ...
        return transformer.clean_mask, "Gaussian Blur (Transformation.py)"
